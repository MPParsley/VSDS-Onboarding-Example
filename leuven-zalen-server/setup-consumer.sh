...
echo ""
echo "# Count triples in Fuseki:"
echo "  curl -X POST ${FUSEKI_URL}/${DATASET_NAME}/sparql \\"
echo "    --data-urlencode 'query=SELECT (COUNT(*) as ?count) WHERE { ?s ?p ?o }' \\"
echo "    -H 'Accept: application/sparql-results+json'"
echo ""
echo "# Query zalen in Fuseki:"
echo "  curl -X POST ${FUSEKI_URL}/${DATASET_NAME}/sparql \\"
echo "    --data-urlencode 'query=PREFIX schema: <https://schema.org/> SELECT ?zaal ?name WHERE { ?zaal schema:name ?name }' \\"
echo "    -H 'Accept: application/sparql-results+json'"
echo ""
